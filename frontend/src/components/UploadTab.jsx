...
import { FileText, Upload, AlertCircle, ChevronRight, Trash2, RefreshCw, Download, Sparkles, Zap } from 'lucide-react';
          ...
          try {
            ...
            if (payload.progress === 100) {
              setTimeout(async () => {
                ...
                const loadedQs = await loadQuestionsForSubject(upCode, upSem, false, {
                  code: upCode,
                  name: upName,
                  semester: upSem,
                  regulation: upReg
                });
                if (handleAutoGenerateDualSets && loadedQs && loadedQs.length > 0) {
                  handleAutoGenerateDualSets(loadedQs);
                }
              ...
              <div style={{ display: 'flex', gap: '0.75rem', marginTop: '0.5rem', flexWrap: 'wrap' }}>
                <button 
                  ...
                  onClick={async () => {
                    ...
                    const qs = await loadQuestionsForSubject(existingSubject.code, existingSubject.semester, false, existingSubject);
                    const pool = (qs && qs.length > 0) ? qs : existingQuestions;
                    if (handleAutoGenerateDualSets) {
                      handleAutoGenerateDualSets(pool);
                    }
                    setActiveTab('questions');
                  }}
                >
                  <Sparkles size={16} /> ⚡ Auto-Generate 2 Sets & Preview
                </button>
                <button 
                  type="button" 
                  className="btn" 
                  style={{ flex: '1 1 140px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem', border: '1px solid var(--border-color)', background: '#f8fafc' }}
                  onClick={() => {
                    setSelectedSubCode(existingSubject.code);
                ...
                <button 
                  type="button" 
                  className="btn" 
                  style={{ flex: '0 0 auto', background: 'transparent', border: '1px solid #dcdcdc', color: '#666', fontSize: '0.82rem' }}
